...
const AIIdeaGenerator: React.FC = () => {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    const [fetchedIdeas] = await Promise.all([
      generateContentIdeas(niche, platform),
  ...
  return (
    <section id="ai-tool" className="py-20 bg-slate-900 border-y border-slate-800">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <h2 className="text-3xl font-bold text-white mb-4">Content Strategy Generator</h2>
        <p className="text-slate-400 mb-10 max-w-2xl mx-auto">
          Stuck on ideas? Let our AI suggest viral concepts for your next edit.
        </p>

        <form onSubmit={handleGenerate} className="flex flex-col sm:flex-row gap-4 justify-center max-w-2xl mx-auto mb-12">
          <input
            type="text"
        ...
        {ideas.length > 0 && (
          <div className="grid gap-6 md:grid-cols-3 text-left">
            {ideas.map((idea, index) => (
              <div key={index} className="bg-slate-950 p-6 rounded-xl border border-slate-800 hover:border-teal-500/30 transition-colors">
                ...
                <p className="text-sm text-slate-400 mb-4">{idea.concept}</p>
                <div className="text-xs bg-slate-900 p-3 rounded border border-slate-800 text-teal-300">
                  <span className="font-bold text-slate-500 uppercase block mb-1">Hook:</span>
                  "{idea.hook}"
                </div>
              </div>
            ))}
